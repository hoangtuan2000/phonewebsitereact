...
const OrdersProcessed = () => {
  ...
  const getAllOrdersProcessed = () => {
    Axios.get(URL + '/ordersAdmin/getAllOrdersProcessed')
    .then((res) => {
      if (res.data.getAllOrdersProcessedStatus && res.data.getAllOrdersProcessedData.length > 0) {
        setOrders(res.data.getAllOrdersProcessedData)

      } else if (res.data.getAllOrdersProcessedStatus && res.data.getAllOrdersProcessedData.length <= 0) {
        document.getElementById('showAllOrders').innerHTML = '<h3>hiện tại chưa có đơn hàng</h3>'

      } else if (!res.data.getAllOrdersProcessedStatus) {
        document.getElementById('showAllOrders').innerHTML = `<h3>${res.data.getAllOrdersProcessedMessage}</h3>`

      }

    })
    .catch((err) => {
      console.log('/ordersAdmin/getAllOrdersProcessed', err);
  ...
  return (
    <>
      <div className="home">
        ...
        <div className="homeContainer">
          ...
          <div id='showAllOrders' style={{ height: 500, width: '100%', padding: '10px' }}>
            <div className='mb-2'>
              <h6 className='float-start'>Tất Cả Đơn Hàng Đã Xử Lý</h6>
      ...
      <ShowOrderInfoDialog
        ...
        orderInfo={orderInfo}
        getAllOrders={getAllOrdersProcessed}
