...
const AllStaffAccounts = () => {
  ...
  // get all accounts
  useEffect(() => {
    Axios.get(URL + '/accountsAdmin/getAllStaffAccounts')
      .then((res) => {
        if (res.data.getAllStaffAccountsStatus && res.data.getAllStaffAccountsData.length > 0) {
          setAccounts(res.data.getAllStaffAccountsData)

        } else if (res.data.getAllStaffAccountsStatus && res.data.getAllStaffAccountsData.length <= 0) {
          document.getElementById('showAllAccounts').innerHTML = '<h3>hiện tại chưa có tài khoản</h3>'

        } else if (!res.data.getAllStaffAccountsStatus) {
          document.getElementById('showAllAccounts').innerHTML = `<h3>${res.data.getAllStaffAccountsMessage}</h3>`

        }

      })
      .catch((err) => {
        console.log('/accountsAdmin/getAllStaffAccounts', err);
      ...
      <div className="home">
        ...
        <div className="homeContainer">
          ...
          <div id='showAllAccounts' style={{ height: 500, width: '100%', padding: '10px' }}>
            <div className='mb-2'>
              <h6 className='float-start'>Tài Khoản Nhân Viên</h6>
...
export default AllStaffAccounts;
