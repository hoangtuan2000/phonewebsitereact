...
const OrdersTransported = () => {
  ...
  const getAllOrdersTransported = () => {
    Axios.get(URL + '/ordersAdmin/getAllOrdersTransported')
    .then((res) => {
      if (res.data.getAllOrdersTransportedStatus && res.data.getAllOrdersTransportedData.length > 0) {
        setOrders(res.data.getAllOrdersTransportedData)

      } else if (res.data.getAllOrdersTransportedStatus && res.data.getAllOrdersTransportedData.length <= 0) {
        document.getElementById('showAllOrders').innerHTML = '<h3>hiện tại chưa có đơn hàng</h3>'

      } else if (!res.data.getAllOrdersTransportedStatus) {
        document.getElementById('showAllOrders').innerHTML = `<h3>${res.data.getAllOrdersTransportedMessage}</h3>`

      }

    })
    .catch((err) => {
      console.log('/ordersAdmin/getAllOrdersTransported', err);
  ...
  return (
    <>
      <div className="home">
        ...
        <div className="homeContainer">
          ...
          <div id='showAllOrders' style={{ height: 500, width: '100%', padding: '10px' }}>
            <div className='mb-2'>
              <h6 className='float-start'>Tất Cả Đơn Hàng Đang Vận Chuyển</h6>
      ...
      <ShowOrderInfoDialog
        ...
        orderInfo={orderInfo}
        getAllOrders={getAllOrdersTransported}
...
export default OrdersTransported;
