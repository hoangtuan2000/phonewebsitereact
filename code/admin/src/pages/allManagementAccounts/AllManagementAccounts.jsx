...
const AllManagementAccounts = () => {
  ...
  // get all accounts
  useEffect(() => {
    Axios.get(URL + '/accountsAdmin/getAllManagementAccounts')
      .then((res) => {
        if (res.data.getAllManagementAccountsStatus && res.data.getAllManagementAccountsData.length > 0) {
          setAccounts(res.data.getAllManagementAccountsData)

        } else if (res.data.getAllManagementAccountsStatus && res.data.getAllManagementAccountsData.length <= 0) {
          document.getElementById('showAllAccounts').innerHTML = '<h3>hiện tại chưa có tài khoản</h3>'

        } else if (!res.data.getAllManagementAccountsStatus) {
          document.getElementById('showAllAccounts').innerHTML = `<h3>${res.data.getAllManagementAccountsMessage}</h3>`

        }

      })
      .catch((err) => {
        console.log('/accountsAdmin/getAllManagementAccounts', err);
      ...
      <div className="home">
        ...
        <div className="homeContainer">
          ...
          <div id='showAllAccounts' style={{ height: 500, width: '100%', padding: '10px' }}>
            <div className='mb-2'>
              <h6 className='float-start'>Tài Khoản Quản Trị</h6>
...
export default AllManagementAccounts;
