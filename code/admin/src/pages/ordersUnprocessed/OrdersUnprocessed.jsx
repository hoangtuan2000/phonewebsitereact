...
const OrdersUnprocessed = () => {
  ...
  const getAllOrdersUnprocessed = () => {
    Axios.get(URL + '/ordersAdmin/getAllOrdersUnprocessed')
    .then((res) => {
      if (res.data.getAllOrdersUnprocessedStatus && res.data.getAllOrdersUnprocessedData.length > 0) {
        setOrders(res.data.getAllOrdersUnprocessedData)

      } else if (res.data.getAllOrdersUnprocessedStatus && res.data.getAllOrdersUnprocessedData.length <= 0) {
        document.getElementById('showAllOrders').innerHTML = '<h3>hiện tại chưa có đơn hàng</h3>'

      } else if (!res.data.getAllOrdersUnprocessedStatus) {
        document.getElementById('showAllOrders').innerHTML = `<h3>${res.data.getAllOrdersUnprocessedMessage}</h3>`

      }

    })
    .catch((err) => {
      console.log('/ordersAdmin/getAllOrdersUnprocessed', err);
  ...
  return (
    <>
      <div className="home">
        ...
        <div className="homeContainer">
          ...
          <div id='showAllOrders' style={{ height: 500, width: '100%', padding: '10px' }}>
            <div className='mb-2'>
              <h6 className='float-start'>Tất Cả Đơn Hàng Chưa Xử Lý</h6>
      ...
      <ShowOrderInfoDialog
        ...
        orderInfo={orderInfo}
        getAllOrders={getAllOrdersUnprocessed}
...
export default OrdersUnprocessed;
