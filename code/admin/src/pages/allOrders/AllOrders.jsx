...
const AllOrders = () => {
  ...
  const getAllOrders = () => {
    ...
    Axios.get(URL + '/ordersAdmin/getAllOrders')
      .then((res) => {
        if (res.data.getAllOrdersStatus && res.data.getAllOrdersData.length > 0) {
          setOrders(res.data.getAllOrdersData)

        } else if (res.data.getAllOrdersStatus && res.data.getAllOrdersData.length <= 0) {
          document.getElementById('showAllOrders').innerHTML = '<h3>hiện tại chưa có đơn hàng</h3>'

        } else if (!res.data.getAllOrdersStatus) {
          document.getElementById('showAllOrders').innerHTML = `<h3>${res.data.getAllOrdersMessage}</h3>`

        }

      })
      .catch((err) => {
        console.log('/ordersAdmin/getAllOrders', err);
      ...
      <div className="home">
        ...
        <div className="homeContainer">
          ...
          <div id='showAllOrders' style={{ height: 500, width: '100%', padding: '10px' }}>
            <div className='mb-2'>
              <h6 className='float-start'>Tất Cả Đơn Hàng</h6>
