...
const OrdersSuccessDelivery = () => {
  ...
  const getAllOrdersSuccessDelivery = () => {
    Axios.get(URL + '/ordersAdmin/getAllOrdersSuccessDelivery')
    .then((res) => {
      if (res.data.getAllOrdersSuccessDeliveryStatus && res.data.getAllOrdersSuccessDeliveryData.length > 0) {
        setOrders(res.data.getAllOrdersSuccessDeliveryData)

      } else if (res.data.getAllOrdersSuccessDeliveryStatus && res.data.getAllOrdersSuccessDeliveryData.length <= 0) {
        document.getElementById('showAllOrders').innerHTML = '<h3>hiện tại chưa có đơn hàng</h3>'

      } else if (!res.data.getAllOrdersSuccessDeliveryStatus) {
        document.getElementById('showAllOrders').innerHTML = `<h3>${res.data.getAllOrdersSuccessDeliveryMessage}</h3>`

      }

    })
    .catch((err) => {
      console.log('/ordersAdmin/getAllOrdersSuccessDelivery', err);
  ...
  return (
    <>
      <div className="home">
        ...
        <div className="homeContainer">
          ...
          <div id='showAllOrders' style={{ height: 500, width: '100%', padding: '10px' }}>
            <div className='mb-2'>
              <h6 className='float-start'>Tất Cả Đơn Hàng Giao Hàng Thành Công</h6>
      ...
      <ShowOrderInfoDialog
        ...
        orderInfo={orderInfo}
        getAllOrders={getAllOrdersSuccessDelivery}
...
export default OrdersSuccessDelivery;
