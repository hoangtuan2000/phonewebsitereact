...
const Smartphones = () => {
  ...
  // get all products
  useEffect(() => {
    Axios.get(URL + '/productsAdmin/getAllSmartphones')
      .then((res) => {
        if (res.data.getAllSmartphonesStatus && res.data.getAllSmartphonesData.length > 0) {
          setProducts(res.data.getAllSmartphonesData)

        } else if (res.data.getAllSmartphonesStatus && res.data.getAllSmartphonesData.length <= 0) {
          document.getElementById('showAllProducts').innerHTML = '<h3>hiện tại chưa có sản phẩm</h3>'

        } else if (!res.data.getAllSmartphonesStatus) {
          document.getElementById('showAllProducts').innerHTML = `<h3>${res.data.getAllSmartphonesMessage}</h3>`

        }

      })
      .catch((err) => {
        console.log('/productsAdmin/getAllSmartphones', err);
        ...
        return (
          <>
            ...
            <Tooltip TransitionComponent={Zoom} title="Cập Nhật" followCursor>
              <IconButton color="warning" onClick={() => navigate(`/updateProduct/${params.row.id}?productType=smartphone`)}>
                <UpgradeIcon />
              </IconButton>
            </Tooltip>
          </>
      ...
      <div className="home">
        ...
        <div className="homeContainer">
          ...
          <div id='showAllProducts' style={{ height: 500, width: '100%', padding: '10px' }}>
            <div className='mb-2'>
              <h6 className='float-start'>Tất Cả Điện Thoại</h6>
